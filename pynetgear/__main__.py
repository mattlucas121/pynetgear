...
DEFAULT_URL_PATH = '/soap/server_sa/'
ALTERNATIVE_URL_PATH = '/DEV_device_info.htm'
...
class Netgear(object):
    ...
    def __init__(
        ...
        if url:
            self.soap_url = url + DEFAULT_URL_PATH
        else:
            if not host:
                host = DEFAULT_HOST
            if not port:
    ...
    def _set_alternative_url(self):
        self.soap_url = self.soap_url.replace(DEFAULT_URL_PATH, ALTERNATIVE_URL_PATH)
    ...
    def _make_request(
        ...
        try:
            ...
            # Catch ConnectionError and ReadTimeout which can be caused by improper URL path
            _LOGGER.info("Unable to find endpoint, trying alternative endpoint...")
            self._set_alternative_url()
            response = requests.post(self.soap_url, headers=headers,
                                     data=message, timeout=30, verify=False)
        try:
